...
//
// Step 4: loads still publish exactly one `PlacedClip` with `startSampleOnTimeline == 0` so
//   Transport’s clip-relative playhead and the old Phase 1 behavior match.
//
...
public:
    ...
    // (jassert in .cpp). `startSampleOnTimeline` is in device samples, aligned with the
    // timeline rules in steering docs; Step 4 only uses 0.
