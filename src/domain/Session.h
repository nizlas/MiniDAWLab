...
//
// THREAD MODEL
//   • replaceClipFromFile / clearClip: [Message thread] in Step 4; they may block (decode on load).
//   • loadSessionSnapshotForAudioThread: [Audio thread] or [Message thread] — acquire-load;
//     refcount only on the hot path; no decode.
//   • getCurrentClip: [Message thread] convenience: narrows the snapshot to the single-clip
//     material the Phase 1 waveform still expects; the pointer is into memory held by the
//     current snapshot, which `Session` keeps published.
//
// OWNERSHIP
...
public:
    ...
    // [Message thread] Decode file → on success, release-store a *new* snapshot (one
    // `PlacedClip` at start 0). On failure, **do not** replace the pointer — the previous
    // snapshot (if any) remains what playback and the UI use.
    ...
    // [Message thread] Phase 1 / Step 4 UI: first and only `PlacedClip`’s `AudioClip`, or
    // nullptr. When the snapshot model grows, callers that need “the” visible clip will follow
    // product rules (not this narrow helper).
