...
Readability refactors performed under the **Readable method bodies** tier (see [`docs/CODE_DOCUMENTATION_RUBRIC.md`](CODE_DOCUMENTATION_RUBRIC.md)) do not require a steering-document update, because they are bounded to the allowances under **Readability refactors allowed during documentation passes** there; any refactor outside those allowances is an architecture change and must follow the normal escalation process.
...
Code-level documentation for **central** `src/` files is mandatory — not stylistic fluff. The **single authoritative** definition is **[`docs/CODE_DOCUMENTATION_RUBRIC.md`](CODE_DOCUMENTATION_RUBRIC.md)**.

### In-code documentation (day-to-day summary)

- **Six tiers**: file header, class documentation, contracts for non-trivial / thread-sensitive methods, explicit **audio-thread** markers where required, terse **JUCE-usage notes** where the API intent is opaque, and **readable method bodies** (top-down chunks, role-named locals, plain-language branch meaning where mechanics would hide intent).
- **Pedagogical default**: comments explain *role in the system*, not narration of syntax; internal helpers that encode product-visible rules meet the same bar as large methods when they carry that meaning.
- **Bounded readability-only refactors** are allowed only as specified under **Readability refactors allowed during documentation passes** in the rubric — behavior-preserving; anything outside that list is an architecture change.
- **Hard gate**: a phase that **adds or meaningfully changes** central files is incomplete if those files violate the six tiers (unless an explicit exception is documented in the phase report).
- **Documentation-pass evidence** must include a real method body + branch/helper example, not a summary alone — see rubric § **Documentation-pass review output**.

**When to read the rubric in full:** while editing meaningful `src/` code or closing a documentation/readability slice. **Trivial steering-only Markdown** with **no** substantial `src/` delta need not reread the entire rubric; if the slice still touches central code, apply the gate as usual (see rubric **Exemptions** for drive-by minimal edits).
