...
- **UI:** `TimelineViewportModel` holds `visibleStartSamples_` + `visibleLengthSamples_`; `setVisibleLengthIfUnset(30s)` and default arrangement **60s** (when stored==0 and content==0) are seeded in `Main` from device sample rate. Ruler and lanes map `[visibleStart, visibleStart+length)` to width. Mouse wheel (ruler and lane stack) pans by `visibleLength/8` per event; not during in-flight move/trim. Playhead/seek **clamp to arrangement extent**; playhead is drawn only when inside the visible window.
